...
def build_event(event_prefix, feature_scale, layerwise=True, modulewise=True,
                inputedges=False, skewed=False, noise=False, min_pt=None, **kwargs):
    ...
    # Get edge weight
    # FIXME: No weights of tracks in STT data yet, skipping it.
    ...
    **kwargs
    ...
    try:
        ...
        if not os.path.exists(filename) or overwrite:
            ...
            feature_scale = [100, np.pi, 100]  # ADAK: [cm] to [m]
            ...
            data = Data(
                x=torch.from_numpy(X).float(),  # ADAK: X[:2] if X=[r, phi] else X
            ...
            # ADAK: Add 'isochrone/energydep' etc as cell_data
            logging.info("Getting cell info")

            if kwargs['cell_info']:
                data.cell_data = torch.from_numpy(isochrone).float()
